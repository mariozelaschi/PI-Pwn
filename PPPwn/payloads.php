		...
		if (strtolower($y) == "payloads") {
			...
			if ($ret == 0 && count($pldata) > 0) {
				foreach ($pldata as $z) {
					...
					$haspl = 1;
					print("
                        <button name=\"payload\" value=".urlencode('/media/pwndrives/'.$x.'/'.$y.'/'.$z).">".$z."</button>
                        &nbsp;");
					$cnt++;
					if ($cnt >= 4) {
						print("
...
if ($ret == 0 && count($sdir) > 0) {
	foreach ($sdir as $a) {
		if (str_ends_with($a, ".bin") || str_ends_with($a, ".elf")) {
			$haspl = 1;
			print("
                        <button name=\"payload\" value=".urlencode('/boot/firmware/PPPwn/payloads/'.$a).">".$a."</button>
                        &nbsp;");
			$cnt++;
			if ($cnt >= 4) {
				print("
...
print("
					    <button name=\"reload\" value=\"reload\">Reload page</button>");
done:
print("
                    </form>
                </td>
        ...
        <br><br>
        <center>
            Place payloads in a folder called \"<b>payloads</b>\" on a USB drive and plug it in to the Pi.
            <br>
            You must also enable the binloader server in GoldHEN.
            <br>
            <form method=\"post\">
