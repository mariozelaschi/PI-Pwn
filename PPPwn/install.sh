...
while true; do
    read -p "\033[36mDo you want to use the Python (slower) PPPwn version? (Y|N): \033[0m" pypwnopt
    case $pypwnopt in
    [Yy]*)
...
local_root=/boot/firmware/PPPwn" | sudo tee /etc/vsftpd.conf
        ...
        while true; do
        read -p "$(printf '\r\n\033[36mDo you want to set the \033[36mroot\033[36m account password now?\r\n\r\n\033[36m(Y|N)?: \033[0m')" rapw
        case $rapw in
            [Yy]*)
            sudo passwd root
        ...
        echo -e '\033[32mFTP installed successfully\033[0m'
        break;;
    [Nn]*)
        echo -e '\033[35mSkipping FTP installation\033[0m'
        break;;
    *)
        echo -e '\033[31mPlease answer Y or N\033[0m';;
    esac
...
public=yes' | sudo tee /etc/samba/smb.conf
        ...
        echo -e '\033[32mSamba installed successfully\033[0m'
        break;;
    [Nn]*)
        echo -e '\033[35mSkipping Samba installation\033[0m'
        break;;
    *)
        echo -e '\033[31mPlease answer Y or N\033[0m';;
    esac
...
while true; do
    read -p "$(printf '\r\n\r\n\033[36mConfiguration file found. Do you want to change the stored settings?\033[36m (Y|N)?: \033[0m')" conf
    case $conf in
    [Yy]*)
        break;;
    [Nn]*)
...
case $dlnk in
    [Yy]*)
    DTLNK="true"
    echo -e '\033[32mConsole shutdown detection enabled\033[0m'
    break;;
    [Nn]*)
    echo -e '\033[35mConsole shutdown detection disabled\033[0m'
    DTLNK="false"
    break;;
    *)
    echo -e '\033[31mPlease answer Y or N\033[0m';;
...
case $pppq in
    ...
    INET="false"
    while true; do
        read -p "$(printf '\r\n\r\n\033[36mDo you want the Pi to shut down after successful exploitation?\r\n\r\n\033[36m(Y|N)?: \033[0m')" pisht
        case $pisht in
        [Yy]*)
            SHTDN="true"
            echo -e '\033[32mThe Pi will shut down after success\033[0m'
            break;;
        [Nn]*)
            echo -e '\033[35mThe Pi will not shut down after success\033[0m'
            SHTDN="false"
            break;;
        *)
            echo -e '\033[31mPlease answer Y or N\033[0m';;
...
case $usbeth in
    [Yy]*)
    USBE="true"
    echo -e '\033[32mUSB to Ethernet adapter will be used\033[0m'
    break;;
    [Nn]*)
    echo -e '\033[35mUSB to Ethernet adapter will NOT be used\033[0m'
    USBE="false"
    break;;
    *)
    echo -e '\033[31mPlease answer Y or N\033[0m';;
...
while true; do
read -p "$(printf '\r\n\r\n\033[36mWould you like to change the timeout for PPPwn to restart if it hangs?\r\n(Default is 5 minutes)\r\n\r\n\033[36m(Y|N)?: \033[0m')" tmout
case $tmout in
    [Yy]*)
    while true; do
...
case $fwset in
    ...
    break;;
    [Nn]*)
    echo -e '\033[35mUsing the default firmware version: 11.00\033[0m'
    FWV="11.00"
    break;;
    *)
    echo -e '\033[31mPlease answer Y or N\033[0m';;
...
while true; do
read -p "$(printf '\r\n\r\n\033[36mDo you want to use the original IPv6 address from PPPwn?\r\n\r\n\033[36m(Y|N)?: \033[0m')" uoipv
case $uoipv in
    [Yy]*)
    IPV="true"
...
while true; do
    read -p "$(printf '\r\n\r\n\033[36mDo you want the Pi to act as a USB flash drive for the console?\r\n\r\n\033[36m(Y|N)?: \033[0m')" vusb
    case $vusb in
    [Yy]*)
        echo -e '\033[32mThe Pi will mount as a USB drive\n\033[33mNote: You must plug the Pi into the console USB port using the \033[35mUSB-C\033[33m port of the Pi,\nand the USB drive in the Pi must contain a folder named \033[35mpayloads\033[0m'
        sudo sed -i "s^dtoverlay=dwc2^^g" /boot/firmware/config.txt
        echo 'dtoverlay=dwc2' | sudo tee -a /boot/firmware/config.txt
        VUSB="true"
        break;;
    [Nn]*)
        echo -e '\033[35mThe Pi will not mount as a USB drive\033[0m'
        sudo sed -i "s^dtoverlay=dwc2^^g" /boot/firmware/config.txt
        VUSB="false"
        break;;
    *)
...
while true; do
read -p "$(printf '\r\n\r\n\033[36mWould you like to change the hostname?\r\n(Default is pppwn)\r\n\r\n\033[36m(Y|N)?: \033[0m')" hstset
case $hstset in
    [Yy]*)
    while true; do
