...
if (isset($_POST['save'])) {
	...
	exec('sudo iptables -t nat -S PREROUTING 2>/dev/null | grep "192\\.168\\.2\\." | sed "s/^-A/-D/" | while read rule; do sudo iptables -t nat $rule 2>/dev/null; done');
	exec('sudo iptables -t nat -S POSTROUTING 2>/dev/null | grep "192\\.168\\.2\\." | sed "s/^-A/-D/" | while read rule; do sudo iptables -t nat $rule 2>/dev/null; done');
	exec('sudo iptables -t nat -F PPPWN 2>/dev/null');
	exec('sudo iptables -t nat -X PPPWN 2>/dev/null');
...
if (str_starts_with(trim(implode($pidata)), "Raspberry Pi 4") || str_starts_with(trim(implode($pidata)), "Raspberry Pi 5")) {
	...
	if ($vmusb == "true" && ! empty($dwcval)) {
		print("
	        <button name=\"remount\">Remount USB</button> &nbsp;");
	}
}

print("
            <button name=\"network\">Network Settings</button> &nbsp;
            <button name=\"restart\">Restart PPPwn</button> &nbsp;
...
for ($x =1; $x<=5;$x++) {
	if ($timeout == $x."m") {
		print("
					        <option value=\"".$x."m\" selected>".$x."m</option>");
	} else {
		print("
...
print("
                        ...
                        <label for=\"ledact\">&nbsp; Led activity</label>
                        <br><br>");
...
if (implode($pypdata) == "install ok installed") {
	...
	if ($upypwn == "false") {
		print("
                        &nbsp;
                        <a href=\"pconfig.php\" style=\"text-decoration:none;\">
                            <label id=\"pconfig\">PPPwn C++ Options</label>
                        </a>");
	}
	print("
				        <br>");
...
print("
                        <br>
                        <input type=\"checkbox\" name=\"oipv\" value=\"".$oipv."\" ".$cval.">
                        <label for=\"oipv\">
                            &nbsp;Use original source ipv6
                            <label style=\"font-size:12px; padding:4px;\">(fe80::4141:4141:4141:4141)</label>
                        </label>
                        <br>");
...
print("
                        ...
                        <label for=\"restmode\">
                            ...
                            <label style=\"font-size:12px; padding:4px;\">(useful for rest mode)</label>
                        </label>
                        <br>");
...
if ($ppdbg == "true") {
	print("
                        ...
                        &nbsp;
                        <a href=\"javascript:void(0);\" style=\"text-decoration:none;\">
                            <label id=\"pwnlog\">Open Log Viewer</label>
                        </a>
                        <br>");
} else {
	print("
...
print("
                        <br>
                        <input type=\"checkbox\" name=\"ddns\" value=\"".$ddns."\" ".$cval.">
...
if (isset($_POST['update'])) {
	$localver_raw = @file_get_contents('/boot/firmware/ver');
	$localver = ($localver_raw === false) ? 'Unknown' : trim($localver_raw);
	$remotever_raw = @file_get_contents('https://raw.githubusercontent.com/mariozelaschi/PI-Pwn/main/ver');
